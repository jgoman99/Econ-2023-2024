...
\begin{parts}
    \part What is $E[Y_n]$?
    \begin{solution}
        $$\bar{X_n} \sim N(0,1/n) \implies \sqrt{n} \bar{X_n} \sim N(0,1)$$
        \begin{align*}
            E[Y_n] &= P(\bar{X_n} \geq 1) \\
            &= P(\sqrt{n}\bar{X_n} \geq \sqrt{n})\\
            &= 1 - \Phi(\sqrt{n})
        \end{align*}
    \end{solution}
    \part What is $Var(Y_n)$?
    \begin{solution}
        Note that $Y_n$ is a Bernoulli random variable.\\
        So $Var(Y_n) = p(1-p) = (1 - \Phi(\sqrt{n}))(\Phi(\sqrt{n}))$
    \end{solution}
    \part What is the probability limit of $Y_n$?
    \begin{solution}
        Markov's inequality is $P(x \geq a) \leq \frac{E[X]}{a}$:
        \begin{align*}
            P(Y_n \geq 1) &\leq \frac{E[Y_n]}{1} = 1 - \Phi(\sqrt{n})
        \end{align*}
        As $n \to \infty$, $1 - \Phi(\sqrt{n}) \to 0$. So $Y_n \xrightarrow{p} 0$.
    \end{solution}
\end{parts}
