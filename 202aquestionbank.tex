...
\end{solution}
    ...
    \begin{parts}
        ...
        \begin{solution}
            TODO: Check\\\\
            Yes, the equilibrium allocations for this economy are the solution to the social planner's problem as the taxes are not distortionary. The social planner's problem is:
            \begin{align*}
                V(z,g,k,h_{-1}) &= \max_{c,k',h} \log{c} + A \log L_t + \beta E[V(z',g',k',h)|z,g]\\
                s.t.\ (g + c + k' &= e^{z} k^\theta h^{1-\theta} + (1-\delta)k)\\
                \text{and } L_t &= a(1-h) + (1-a)(1-h_{-1})\\
                \text{and } z' &= \rho_z z + \epsilon'^{z}\\
                \text{and } g' &= \rho^g(g-\bar{g}) + \epsilon'^g + \bar{g}
            \end{align*}
            TODO: Understand $\rho_g$ better.
        \end{solution}
        ...
        \begin{solution}
            TODO: transversality condition.

            Sketch: Take FOCs for c,k',h, envelope condition, resource constraint, and equation for $L_t$.
        \end{solution}
        ...
        \begin{solution} TODO\\\\
            ...
            \begin{enumerate}
                \item The household's problem:
                \begin{align*}
                    \max_{c,k',h} &\log c + A \log L \\
                    s.t.\ & c + k' + \tau = r(z,g,K,H)k + w(z,g,K,H)h + (1-\delta)k\\
                    \text{and } z' &= \rho_z z + \epsilon'^{z}\\
                    g' &= \rho^g(g-\bar{g}) + \epsilon'^g + \bar{g}\\
                    K' &= G(z,g,K,H)\\
                    H &= P(z,g,K,H_{-1})
                \end{align*}
                \item The firm's problem:
                \begin{align*}
                    \max_{k^f,h^f} e^z k^{f\theta} h^{f(1-\theta)} - r(z,g,K,H)k^f - w(z,g,K,H)h^f
                \end{align*}
                \item A set of household decision rules, $c(z,g,K,H), k'(z,g,K,H), h(z,g,K,H)$
                \item A set of firm decision rules
            \end{enumerate}
        \end{solution}
